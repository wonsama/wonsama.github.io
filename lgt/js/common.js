...
const get_all_deligations = async (account, token='', cmd='delegate_vesting_shares', limit=10000) => {
  ...
  let sp_gopower = await get_vp_with_price('gopower');
  let sp_gotogether = await get_vp_with_price('gotogether');
  ...
  // 토큰 정보를 추가한다
  let tholders = [];
  let total_stake_lgt = 0;
  if(token!=''){
    console.log(`token_holders_all : start`);
    let holders = await token_holders_all(token.toUpperCase());
    total_stake_lgt = holders.reduce((acc,curr)=>{
      return acc + parseFloat(curr.stake);
    }, 0);
    total_stake_lgt = parseFloat(total_stake_lgt.toFixed(4));
    console.log(`token_holders_all : end`);

    for(let c of holders){
      var vp = parseFloat((Math.min( ( (c.stake * 100 ) / total_stake_lgt ) * FULL_VOTE_SIZE , 100)).toFixed(2));
  ...
  for(let [k,v] of Object.entries(delegator_obj)){
    ...
    stake_sum= stake_sum + parseFloat(stake);
  }
  ...
  for(let [k,v] of Object.entries(delegator_obj)){
    ...
    let daily = sp>0?(sp*0.18) / 365:0;
    daily = parseFloat(daily.toFixed(3)); // LGT airdrop
    daily_steem = (stake / parseFloat(stake_sum)) * parseFloat((sp_gopower.sp + sp_gotogether.sp)/1e4);
    daily_steem = parseFloat(daily_steem.toFixed(3));
  ...
  votes.reverse();
  votes = votes.filter(([idx,trx])=>trx.op[0]=='vote'&&trx.op[1].voter=='gotogether');
