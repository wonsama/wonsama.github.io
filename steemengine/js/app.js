...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(".data-find").click(function(){
        ...
        if(no==1){
            ...
            let html = '스테이킹+임대-임차</a>';
            $("#search_text").html(_get_nbsp(10)+html+_get_nbsp(10));
        }else if(no==3){
            let html = '잔고+스테이킹+임차</a>';
            $("#search_text").html(_get_nbsp(10)+html+_get_nbsp(10));
        }
...
function refresh_screen(type=1/* 1: 잔고, 2 : 스테이킹, 3 : 잔고 + 스테이킹 */){
    ...
    if(type==1){
        ...
        // 3: 잔고 + 스테이킹 + 임차
        _dataset.sort((a, b) => parseFloat(b.balance) - parseFloat(a.balance) + parseFloat(b.stake) - parseFloat(a.stake)  + parseFloat(b.delegationsOut) - parseFloat(a.delegationsOut));
    }
    ...
    for (let r of _dataset) {
        ...
        if(type==1){ // 벨런스 
            ...
            balance = parseFloat(r.balance)+parseFloat(r.stake)+parseFloat(r.delegationsOut);
        }
        if(balance>0){
    ...
    for (let r of _dataset) {
        ...
        // let gap_show = ''
        // if(parseFloat(_gap)>0){
        //     gap_show = `<br/><span class="pink-text text-darken-3">(${add_comma(parseFloat(r.stake))}+${add_comma(_gap)})</span>`;
        // }else if(parseFloat(_gap)<0){
        //     gap_show = `<br/><span class="blue-text text-darken-4">(${add_comma(parseFloat(r.stake)-_gap)}${add_comma(_gap)})</span>`;
        // }

        let dbal = add_comma(parseFloat(r.balance));
        let dstk = add_comma(parseFloat(r.stake));
        let din = add_comma(parseFloat(r.delegationsIn));
        let dout = add_comma(parseFloat(r.delegationsOut));

        let gap_show = ''
        if(type==2 && din!=0 && din!=0){

            if(parseFloat(r.delegationsOut)>0){
                dstk = add_comma( parseFloat(r.stake) + parseFloat(r.delegationsOut) );
            }

            gap_show += `<br/><span class="black-text text-darken-3">(${dstk}</span>`;
            gap_show += `<span class="pink-text text-darken-3">+${din}</span>`;
            gap_show += `<span class="blue-text text-darken-4">-${dout}</span>`;
            ...
            gap_show += `<span class="black-text text-darken-3">)</span>`;
        }
        if(type==3 && din!=0 && din!=0){
            gap_show += `<br/><span class="black-text text-darken-3">(${dbal}</span>`;
            gap_show += `<span class="pink-text text-darken-3">+${dstk}</span>`;
            gap_show += `<span class="blue-text text-darken-4">+${dout}</span>`;
        ...
        if(r.account=='gaeteol'){
            console.log(r)
        }
        if(r.account=='scotpool.miner'){
            console.log(r)
        }
